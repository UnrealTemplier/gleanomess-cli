...
## 2. Toolchain & Target Platforms

- **Go Toolchain:** `Go 1.27.1` (minimum required toolchain across `go.mod`, CI, and development environments).
- **Dependencies:** Standard library + pure Go dependencies (`golang.org/x/net`, `golang.org/x/image`).
- **Build Constraint:** Strictly **CGO-free** (`CGO_ENABLED=0`).
- **Target Platforms:**
  - **Linux amd64** (primary platform; Fedora x86_64 and `ubuntu-latest`).
...
Automated GitHub Actions CI (`.github/workflows/ci.yml`) runs on every push and pull request to `main` across three matrix runners:
...
## 5. Important Invariants (DO NOT VIOLATE)
...
3. **Resolve relative URLs against the page URL.** Use page URL (or `<base href>` if defined) and strip fragments (`#...`).
4. **`--limit-size` means `max(width, height) >= N`.** If the largest dimension is at least `N`, the image is accepted.
5. **Do not fully decode large images merely to inspect dimensions.** Always use `DecodeConfig` / box headers without loading pixel bitmaps into RAM.
6. **Download into temporary storage before finalizing.** Write to `.gleanomess-*.tmp` files; inspect, validate, and hash; atomically rename only upon success; remove temp file on error or rejection.
7. **URL deduplication must remain concurrency-safe.** Each normalized URL must be downloaded at most once across concurrent workers.
...
10. **Do not download lower-priority candidates unnecessarily after success.** As soon as a candidate in an image slot succeeds and meets size criteria, stop processing that slot immediately.
11. **JSON-LD extraction must only use image-related semantic fields.** Extract only valid image properties (`image`, `contentUrl`, `thumbnailUrl`, `ImageObject`, `@graph`, arrays). Never treat arbitrary strings (e.g. `@context`, `headline`, `author`, `name`) as image candidates.
...
- **Prefer small changes:** Keep changes focused, coherent, and minimal.
- **Keep dependencies minimal:** Prefer standard library; new dependencies must be justified and strictly CGO-free.
- **Add regression tests for important bug fixes:** Maintain high automated test coverage for URL resolution, srcset parsing, image inspection, size filtering, naming, HTTP retries, atomic dedup, and candidate fallback chains.
- **Use `httptest` for HTTP tests:** Mock servers should be self-contained using `httptest.Server`.
- **Run `gofmt`:** Format all code with standard `gofmt -w -s .`.
- **Run validation tools:** Run `go test`, `go vet`, and `go build` regularly during development.
- **Do not rewrite working architecture without reason:** Respect established modularity and invariants.
...
- **Do not silently expand scope.** If a requested feature violates v0.1.x scope, explain why or defer it.
...
- **Update documentation:**
  ...
  - Update `AGENTS.md` when architecture or invariants materially change.
- **Run validation before finishing:** Always verify that quality gates pass cleanly.
